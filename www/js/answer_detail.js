...
function () {
  ...
  */
  ...
  _createClass(AnswerDetail, [{
    ...
    value: function toViewAnswerDetail(type, mode) {
    ...
    */
    ...
    value: function setAnswerDetailTable(mode) {
      $(GROBAL.answer_detail.value.answer_detail_block).append('<div id="answer_table_box">' + '<table id="answer_table">' + '</table>' + '<div class="answer_detail_role">' + '<img id="answer_detail_img" src="">' + '</div>' + '<div class="one_point_start">' + '<div id="one_point_start_button" class="one_point_start_outer">' + '<div class="one_point_start_inner">' + 'One Point' + '</div>' + '<div class="one_point_start_inner_border"></div>' + '</div>' + '</div>' + '</div>');
      ...
      this.common.createDetailBlock(local_answer_id + "_" + local_word_detail_id, GROBAL.answer_detail.value.answer_table, local_text_ja, local_text_en, local_text_en_phonetic, GROBAL.answer_detail.view.voice); // ワンポイントボタンが押下された時のアクション

      $(GROBAL.main.element.one_point_start_button).on("click", {
        answer_detail: this
      }, function (e) {
    ...
    */
    ...
    value: function toViewOnePoint() {
      ...
      this.common.hideParts(GROBAL.answer_detail.value.transition_first); // ワンポイント詳細セット

      this.setOnePointTable(); // 画像セット
      //document.getElementById("restart_img").src="img/restart.png";
      // 表示パーツ変更

      this.common.changeParts(GROBAL.answer_detail.value.transition_second); // 学習履歴オブジェクト更新
    ...
    */
    ...
    value: function setOnePointTable() {
      $(GROBAL.answer_detail.value.one_point_block).append('<div class="one_point_text">' + '<p id="one_point_top">' + '</p>' + '</div>' + '<div id="one_point_question_table_box">' + '<table id="one_point_question_table">' + '</table>' + '<table id="one_point_answer_table">' + '</table>' + '<div class="restart">' + '<div id="restart_button" class="restart_outer">' + '<div class="restart_inner">' + 'はじめからもう一度' + '</div>' + '<div class="restart_inner_border"></div>' + '</div>' + '</div>' + '</div>');
      ...
      $(GROBAL.main.element.restart_button).on("click", {
        answer_detail: this
      }, function (e) {
    ...
    */
    ...
    value: function updateHistoryOb() {
      ...
      that.common.dbRequest.onsuccess = function (e) {
        ...
        var current_day_store = current_day_tran.objectStore("t_current_day");
        var parent_records_store = parent_records_tran.objectStore("t_parent_records");
        var child_records_store = child_records_tran.objectStore("t_child_records"); // 親オブジェクト中身編集

        var parent_key = that.common.talkingId;
        var parent_records_data;
        var parent_upd_flg = true;
        ...
        if (that.common.questionRecords == "") {
          ...
          for (var i = 0; i < that.common.questionRecords.length; i++) {
        ...
        if (parent_upd_flg) {
          var put_parent_records = parent_records_store.put(parent_records_data);
        ...
        var child_key = parent_key + "_" + that.common.questionId;
        var child_records_data;
        var child_upd_flg = true;
        ...
        if (that.common.answerRecords == "") {
          ...
          for (var _i = 0; _i < that.common.answerRecords.length; _i++) {
            if (that.common.answerId == that.common.answerRecords[_i]) {
              child_upd_flg = false;
              break;
            }
          }
        ...
        if (child_upd_flg) {
          var put_child_records = child_records_store.put(child_records_data);
        ...
        if (that.common.talkingId == that.common.currentDay) {
          ...
          var put_current_day = current_day_store.put(current_day_data);
